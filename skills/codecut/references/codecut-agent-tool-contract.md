...
```text
get_project_info -> list_media_assets -> optional import_media -> choose the confirmed transcript path (`transcribe_media`/`build_video_context` for local transcription, or `transcribe_volcengine_media`/`build_volcengine_media_captions` for provider-backed public-source transcription) -> optional inspect_video_range for ambiguous or reframe-sensitive ranges -> Codex writes clip-first EditPlan -> validate_edit_plan -> preview_edit_plan -> apply_edit_plan -> optional build_post_cut_captions -> Codex writes final EditPlan -> validate_edit_plan -> preview_edit_plan -> apply_edit_plan -> verify_timeline -> get_timeline_state -> optional export_project
```

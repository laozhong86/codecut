...
| `scriptwriting` | `skills/codecut-scriptwriting/SKILL.md` | User asks for cover title, video title, hook, voiceover script, spoken-word draft, or de-AI copy cleanup before editing | Topic, audience, proof, platform, or duration gaps would force false claims or unusable spoken timing | No mutation readback; hand off a ScriptwritingBrief only | Separated cover/video titles, voiceover beats, de-AI rewrite log, CodeCut handoff |
...
- Scriptwriting is a loadable upstream content brief, not a timeline execution
  stage. It may feed cover, overlay, caption, voiceover, or planning decisions,
  but it must not be reported as timeline mutation.

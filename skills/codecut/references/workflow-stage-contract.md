...
- Lower execution risk: material download, reference derivation, executor
  ...
  conditions.
- Clearer material judgment: source facts, material understanding, and edit
  planning stay separate so role labels and composition affordances do not turn
  into hidden timeline decisions.
...
- Stronger title ownership: source-backed title judgment lives in a title
  generation skill before edit planning, while runtime code only evaluates
  deterministic title quality when a rubric is provided.
...
| `requirement-intake` | `codecut-requirement-intake` | User brief, known source path or URL, known output constraints | Setup submission, confirmed setup token, `requirement-intake.md`, explicit user answers, assumptions kept separate | "Setup confirmed" or "Missing setup fields" | Two or more blocking fields are missing; setup submission has not passed; confirmed setup token is missing for side effects | `codecut-material-ingest` when source facts are needed; `codecut-reference-template` for reference derivation; `codecut-executor-apply` only when source is already in an executor project |
...
| `material-ingest` | `codecut-material-ingest` | Confirmed setup token, confirmed source paths or remote URLs, workspace project ID | `.codecut-workspace` assets, ffprobe inventory, `material-audit.md` | "Source material ready" or "Source blocked" | Confirmed setup token is missing or invalid; remote source cannot be downloaded or probed; local path is not absolute; media has no positive duration | `codecut-material-understanding` when material roles, script matching, or composition suitability matter; otherwise `codecut-reference-template` or `codecut-edit-planning` |
| `material-understanding` | `codecut-material-understanding` | Material audit, asset manifest, transcript or VideoContext, visual evidence, optional script segments | `material-understanding.json`, `material-understanding.md` | "Material understood" or "Material understanding blocked" | Material audit is missing or stale; required transcript or visual evidence is missing; user is asking for final edit decisions, timeline mutation, or export | `codecut-reference-template` for reference style derivation; otherwise `codecut-edit-planning` before `codecut-executor-apply` |
| `reference-template` | `codecut-reference-template` | Finished reference videos, requested reusable style, evidence availability | `reference-analysis.md`, `template.json`, `template-fields.md`, import result after confirmation | "Template draft ready" or "Template imported" | Speech/copy evidence is missing for an import-ready draft; user has not confirmed import; requested effect is unsupported | Normal `codecut-requirement-intake` and `codecut-material-ingest` when applying the saved template to new material; `codecut-edit-planning` before executor apply; `codecut-executor-apply` only for confirmed import |
| `title-generation` | `codecut-title-generation` | Material evidence, audience, platform, title lane request, proof, existing title when improving | `title-generation-brief.md` with candidate pool, Top 3, selected title, risk, and title_quality handoff | "Title brief ready" or "Title generation blocked" | Material evidence, audience, proof, platform assumption, or claim boundaries are missing enough that titles would invent claims; user requests guaranteed virality | `codecut-edit-planning` consumes selected title; `codecut-scriptwriting` only when voiceover, hook, spoken copy, or De-AI work is also needed |
| `edit-planning` | `codecut-edit-planning` | Passed requirement intake, material audit, material-understanding report when material judgment matters, transcript or VideoContext, visual evidence, reference-template constraints, title-generation brief when title copy is generated, optional platform strategy brief | `intent-route.md`, `editing-decision-ledger.md`, `candidate-clips.json`, strict plan draft, `verification-spec.json`, optional `planning-blockers.md` | "Plan ready" or "Planning blocked" | Required material-understanding, transcript, visual, material, business, caption, platform, title-generation, or template evidence is missing; no candidate clip passes standalone coherence; requested plan cannot be represented safely; more than one primary recipe would be merged | `codecut-executor-apply` only after a strict plan draft and verification spec exist |
...
| `requirement-intake` | `skills/codecut-requirement-intake/SKILL.md` | A new creative job has missing setup fields, remote source, local source, or side-effect intent | Two or more blocking setup fields are missing, setup submission is unavailable, or side-effect token is missing | Confirmed setup token and `00-brief/requirement-intake.md` when a project exists | Confirmed fields, assumptions kept separate, and next owner |
...
| `material-understanding` | `skills/codecut-material-understanding/SKILL.md` | Material roles, content meaning, script matching, replacement, PIP, split-screen, circular talking-head suitability, crop risk, or material risk affects planning | Material audit, transcript, VideoContext, visual context, contact sheet, range inspection, or frame evidence is missing for the claim | `02-inventory/material-understanding.json` and `.md`; no mutation readback | Role labels, content summaries, composition affordances, script-match suggestions, blockers, and next handoff |
| `reference-template` | `skills/codecut-reference-template/SKILL.md` | User asks to learn, import, or apply a reference-derived template | Speech/copy evidence is missing, unsupported effect is requested, or user has not confirmed import | Imported template only after confirmed import; otherwise draft proof only | `reference-analysis.md`, `template.json`, `template-fields.md`, import result |
...
| `title-generation` | `skills/codecut-title-generation/SKILL.md` | User asks for 标题根据素材生成, 顶部固定标题, 封面标题, 视频标题, 标题优化, 爆款标题, fixed top title, cover title, platform title, publish title, or title optimization | Material evidence, proof, audience, platform assumption, or claim boundaries are missing enough that titles would invent claims | `04-planning/title-generation-brief.md` when a project exists; no mutation readback | Candidate pool, Top 3, selected title, formula source, material evidence, risk, and title_quality handoff |
| `scriptwriting` | `skills/codecut-scriptwriting/SKILL.md` | User asks for hook, voiceover script, spoken-word draft, or de-AI copy cleanup before editing; title-only jobs use `codecut-title-generation` first | Topic, audience, proof, platform, or duration gaps would force false claims or unusable spoken timing | No mutation readback; hand off a ScriptwritingBrief only | Voiceover beats, de-AI rewrite log, optional TitleGenerationBrief reference, CodeCut handoff |
| `edit-planning` | `skills/codecut-edit-planning/SKILL.md` | Transcript, VideoContext, candidate clips, decision ledger, or EditPlan/NarratedRemixPlan/CompositeLayoutPlan strategy is needed | Required material-understanding/transcript/visual/material/business/template/network-material evidence is missing or the requested plan cannot be represented safely | No mutation readback; hand off a strict plan draft and verification spec to executor apply | Selected recipe, planning ledger, strict EditPlan, NarratedRemixPlan, or CompositeLayoutPlan draft, verification spec |
...
- Scriptwriting is a loadable upstream content brief, not a timeline execution
  stage. It may feed hook, caption, voiceover, rewrite, or planning decisions,
  but title-only work belongs to `codecut-title-generation` and scriptwriting
  must not be reported as timeline mutation.
- Title-generation is a loadable upstream content brief, not a timeline
  execution stage. Report it as `Stage: title-generation` when title candidates,
  Top 3 ranking, or title quality handoff are active or blocked.
...
- Material-understanding is a loadable stage skill owned by
  `codecut-material-understanding`. Report it as `Stage:
  material-understanding` when material role, matching, or composition
  suitability evidence is active or blocked.
...
- `codecut-material-understanding` owns material role labels, content summaries,
  composition affordances, script-match suggestions, risks, and blockers.
- `codecut-title-generation` owns source-backed fixed top, cover, and platform
  title candidates, Top 3 ranking, and title_quality handoff.

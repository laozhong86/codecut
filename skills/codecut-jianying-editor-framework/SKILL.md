...
- Do not continue after `doctor`, `import-media`, `transcribe`, `apply-plan`, or `get_timeline_state` fails. Fix the failing gate first.
...
5. Run `create-project`, then `doctor`.
...
Before sending any business command, run the executor readiness check for the current implementation. If the only available readiness check is still browser heartbeat based, report that as `P0 blocked: command execution still depends on browser-mounted bridge` rather than asking the user to open or refresh the Browser.
...
3. Use the CLI executor readiness check. If the check still requires a browser-mounted heartbeat, treat it as a known implementation gap.
4. Use `node scripts/codex-bridge.mjs send --project-id <id> --tool get_project_info --args-json '{}'` to confirm the active project.
5. Use `node scripts/codex-bridge.mjs send --project-id <id> --tool list_media_assets --args-json '{}'` to inspect imported media.
6. Select one existing audio/video asset. If none exists and the user has provided an absolute local media path, use `node scripts/codex-bridge.mjs import-media --project-id <id> --file-path /absolute/path/media-file`, then call `list_media_assets` again.
7. If no media exists and no local media path is available, ask the user to import media in Codecut or provide an absolute local file path.
8. Use `node scripts/codex-bridge.mjs transcribe --project-id <id> --media-id <id> --language auto --model-id <model>` when transcript-first editing is needed.
9. If platform output requires a concrete canvas or FPS, call `update_project_settings` explicitly before applying the EditPlan. `EditPlan.target.aspectRatio` is a planning field and does not mutate project settings by itself.
10. Generate the strict implemented EditPlan v1 in Codex. Use only fields supported by `apps/web/src/lib/agent-bridge/edit-plan/schema.ts`.
11. Write the plan to a local JSON file and use `node scripts/codex-bridge.mjs apply-plan --project-id <id> --plan-json-file /absolute/path/edit-plan.json --replace-existing <true|false>`.
12. Use `node scripts/codex-bridge.mjs send --project-id <id> --tool get_timeline_state --args-json '{}'` to verify the applied timeline.
13. Provide the editor URL for human preview and manual adjustment. Do not run export through the local executor until executor export is implemented and the user confirms.

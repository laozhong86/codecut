...
| --- | --- | --- | --- | --- | --- | --- |
| `requirement-intake` | `codecut-requirement-intake` | User brief, known source path or URL, known output constraints | `requirement-intake.md`, explicit user answers, assumptions kept separate | "Setup confirmed" or "Missing setup fields" | Two or more blocking fields are missing; widget or explicit text answers have not passed | `material-ingest` when source facts are needed; `executor-apply` only when source is already in an executor project |
| `material-ingest` | `codecut-material-ingest` | Confirmed or probe-approved source paths, remote URLs, workspace project ID | `.codecut-workspace` assets, ffprobe inventory, `material-audit.md` | "Source material ready" or "Source blocked" | Remote source cannot be downloaded or probed; local path is not absolute; media has no positive duration | `requirement-intake` if material facts expose missing decisions; `evidence-build` when material is ready |

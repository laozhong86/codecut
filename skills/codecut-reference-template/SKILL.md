...
| Speech, subtitles, or visible copy drive the reference | `references/template-contract.md` and the derive workflow below | Transcript/copy evidence is missing for an import-ready draft | `reference-analysis.md`, `template.json`, and `template-fields.md` |
| User confirms import or execution | `../codecut/references/execution-contract.md` success contract table | User has not confirmed the exact draft or executor readiness is unproven | Imported template proof or `get_timeline_state` after applying a plan |
...
Generated files are drafts. The Codecut template library, visible in the
Templates UI and injected into the agent system prompt, is the source of truth
after import. Do not treat a previous `template.json` file in a
...
- Explicit user confirmation before importing a draft into templates.

## Outputs

- Reference evidence summary and unsupported runtime gaps.
- `reference-analysis.md`, `template.json`, and
  `template-fields.md` for a draft package.
- Import result only after the user confirms the exact draft.
- Planning constraints when applying a saved template to new material.
...
- `.codecut-workspace/projects/<projectId>/04-planning/template.json`
- `.codecut-workspace/projects/<projectId>/04-planning/template-fields.md`

The Codecut template library becomes the source of truth only after an
...
| User confirms a generated draft should become reusable | Import the confirmed draft into Codecut templates with `import_template`. |
| User names a saved template while providing new raw material | Read the template from Codecut template context, then run normal Codecut requirement/material/executor stages. |
| User supplies a `template.json` path for use | Treat it as a draft unless the user explicitly confirms import. Do not apply it as system truth directly. |
...
5. Classify the trigger type as one supported template trigger:
...
7. Validate the package against `references/template-contract.md`.
8. Stop and ask the user to confirm whether this exact draft should be imported
   into Codecut templates. Include the template ID, name, trigger types,
...
1. Re-read the exact `template.json` path being imported.
2. Validate it against `references/template-contract.md`.
3. Use `codecut-executor-apply` for bridge readiness and import through
   `import_template` / `import-template` with
   `confirmedByUser: true`.
4. Treat the imported Codecut template as the reusable truth. The draft
   file remains only provenance and must not override the template later.

## Apply Workflow

When the user says to use a named template on new material:

1. Use the matching Codecut template by ID, name, alias, or
   default trigger from the agent's template context.
...
template, ask whether to import it first.

## Output Rules

Read `references/template-contract.md` before writing
`template.json` or `template-fields.md`.
...
`not import-ready` and do not ask for template import confirmation until
...
- successful `import_template` result
- template ID/name/trigger summary from the import result
- statement that the Codecut template library is now the source of truth

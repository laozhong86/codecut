...
describe("SpeechCleanup workflow docs", () => {
	...
	test("document speech cleanup quality gates for talking-head polish", () => {
		...
		const recipe = readFileSync(
			new URL(
				"../../../../../../skills/codecut-edit-planning/references/workflow-recipes/talking-head-polish.md",
				import.meta.url,
			),
			"utf8",
		);
		...
		expect(recipe).toContain('captionMode: "clip-only"');
		expect(recipe).toContain('captionMode: "source-transcript-remap"');
